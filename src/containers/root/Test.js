...
const Test = (props) => {
  ...
  const handleNext = async () => {
    ...
    let shouldPostStartEvent = false;
    let shouldPostSeeEvent = false;
    let shouldPostCompleteEvent = false;
    ...
    if (shouldPostStartEvent) {
      console.log("start");
      await postStartEvent({ formRef });
    }

    if (shouldPostSeeEvent) {
      console.log("see");
      await postSeeEvent({ formRef, seenQuestionId: previousQuestion.id });
    }

    if (shouldPostCompleteEvent) {
      setIsLoading(true);
      console.log("submit response");
      await postResponse({ formRef, responseRef, response: questions });
